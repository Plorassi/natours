...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2FpbGVuZHJhZHMiLCJhIjoiY2twNDN2anJmMWtnbzJvbGdzdzY1Z2dlZiJ9.pegavNzqrfViH8sXBcKR2g';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Extend map bounds to current bounds
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51IvxJxSEyuuNz64cHxTf75e4o68gD4I9ftAfVoXYvlHRoRaiHQWHDJJPevfzlcnlzLrHuNxwIGxn79SARslEz5k1001NaNs3BI'); //Get checkout session from api

            _context.next = 4;
            return (0, _axios.default)("http://127.0.0.1:3000/api/v1/bookings/checkoutSession/".concat(tourId));

          case 4:
            session = _context.sent;
            console.log(session); //use stripe obj to create checkout form + charge credit card
